...
def run_evaluation(train_X, train_y, test_X, test_y, cfg):
    ...
    return precision, recall, fmeasure, auc, mcc, accuracy
...
def edge_list_from_features(features: torch.Tensor, k: int):
    """
    Construct edge list from the feature matrix based on the k-nearest neighbors.
    ...
    Returns:
        edge_list (list): List of edges representing the nearest neighbors.
        weights (list): List of weights for each edge.
    """
    features = features.cpu().numpy()
    assert features.ndim == 2, "The feature matrix should be 2-D."
    assert k <= features.shape[
        0], "The number of nearest neighbors should be less than or equal to the number of vertices."

    tree = cKDTree(features)
    distances, nbr_array = tree.query(features, k=k + 1)  # k+1 because the nearest neighbor is the point itself
    ...
    for i in range(features.shape[0]):
        for j in range(1, k + 1):  # Skip the first neighbor (the point itself)
            edge_list.append((i, nbr_array[i, j]))
            weights.append(distances[i, j])

    return edge_list, weights


def edge_index_from_features(features: torch.Tensor, k: int):
    """
    Construct edge list from the feature matrix based on the k-nearest neighbors.
...
def convert_to_edge_index(edge_list):
    """
    Convert edge list to torch_geometric edge index format.
